    ...
    try:
        ...
        with driver.session() as session:
            if module_id:
                # 获取特定模块的知识图谱
                result = session.run("""
                    MATCH path = (m:glx_Module {id: $module_id})-[:HAS_CHAPTER]->(c:glx_Chapter)-[:HAS_KNOWLEDGE]->(k:glx_Knowledge)
                ...
                # 获取所有模块
                result = session.run("""
                    MATCH (m:glx_Module)-[:HAS_CHAPTER]->(c:glx_Chapter)-[:HAS_KNOWLEDGE]->(k:glx_Knowledge)
                    RETURN m, c, k
                    LIMIT 50
                """)
